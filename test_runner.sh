...
echo "🚀 JACC 3.1 Teams Migration Test Suite"
echo "======================================="
...
if curl -s http://localhost:5000/api/user -b test-admin-cookies.txt | grep -q "error"; then
    echo -e "${RED}❌ Server not running or admin login failed${NC}"
    exit 1
fi

echo -e "${GREEN}✅ Setup complete - Admin session created${NC}"
echo ""

# Test 1: Memory Optimization
echo "🧠 Test 1: Memory Optimization"
MEMORY_USAGE=$(curl -s http://localhost:5000/api/admin/performance -b test-admin-cookies.txt | jq -r '.memory.percentage // empty')
...
echo "🔒 Test 2: Bank-Level Security"
AUDIT_LOGS=$(curl -s http://localhost:5000/api/admin/audit-logs -b test-admin-cookies.txt 2>/dev/null | jq '. | length' 2>/dev/null || echo "0")
...
echo "🤖 Test 3: AI Performance Enhancements"
CACHE_RESPONSE=$(curl -s http://localhost:5000/api/admin/cache-stats -b test-admin-cookies.txt 2>/dev/null)
...
echo "🔍 Test 4: Search Improvements & FAQ Integration"
FAQ_SEARCH=$(curl -s -X POST http://localhost:5000/api/chat/send \
  -H "Content-Type: application/json" \
  -b test-admin-cookies.txt \
  -d '{"message":"What are processing rates for restaurants?","chatId":null,"isNewChat":true}' 2>/dev/null)
...
echo "📊 Test 5: Pinecone Vector Re-indexing"
VECTOR_SEARCH=$(curl -s -X POST http://localhost:5000/api/chat/send \
  -H "Content-Type: application/json" \
  -b test-admin-cookies.txt \
  -d '{"message":"Show me auth.net setup documentation","chatId":null,"isNewChat":true}' 2>/dev/null)
...
echo "🏆 Test 7: Agent Leaderboard Integration"
LEADERBOARD=$(curl -s http://localhost:5000/api/leaderboard -b test-admin-cookies.txt 2>/dev/null)
if echo "$LEADERBOARD" | grep -q "agents\|username\|score"; then
...
echo "📄 Test 8: OCR Word Counting Fix"
# Skip actual file upload test, just check if OCR routes exist
OCR_ROUTES=$(curl -s http://localhost:5000/api/admin/performance -b test-admin-cookies.txt 2>/dev/null)
if [[ $? -eq 0 ]]; then
...
echo "📈 Test 9: Performance Snapshot Generator"
...
echo "🧪 Test 10: Automated Testing Framework"
...
echo "💰 Test 11: TracerPay Rate Standardization"
TRACERPAY_TEST=$(curl -s -X POST http://localhost:5000/api/chat/send \
  -H "Content-Type: application/json" \
  -b test-admin-cookies.txt \
  -d '{"message":"Calculate TracerPay rates for $50,000 restaurant","chatId":null,"isNewChat":true}' 2>/dev/null)
...
echo "📢 Test 12: Marketing Workflow Humanization"
MARKETING_TEST=$(curl -s -X POST http://localhost:5000/api/chat/send \
  -H "Content-Type: application/json" \
  -b test-admin-cookies.txt \
  -d '{"message":"Create marketing strategy for Tracer","chatId":null,"isNewChat":true}' 2>/dev/null)
...
echo "❓ Test 13: FAQ Multiple Answers Combination"
FAQ_COMBO=$(curl -s -X POST http://localhost:5000/api/chat/send \
  -H "Content-Type: application/json" \
  -b test-admin-cookies.txt \
  -d '{"message":"POS options for archery business","chatId":null,"isNewChat":true}' 2>/dev/null)
...
if [[ $FAILED -eq 0 ]]; then
    echo -e "🎉 ${GREEN}ALL TESTS PASSED! JACC 3.1 is ready for Teams deployment!${NC}"
    exit 0
else
    echo -e "⚠️  ${YELLOW}Some tests failed. Check the issues above before Teams migration.${NC}"
    exit 1
fi