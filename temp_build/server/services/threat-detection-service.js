...
class ThreatDetectionService {
    ...
    async detectBruteForce(ipAddress, userId) {
        try {
            ...
            const [failedAttempts] = await db
                .select({ count: count() })
                .from(auditLogs)
                .where(and(eq(auditLogs.eventType, AuditEventType.AUTH_FAILURE), eq(auditLogs.ipAddress, ipAddress), gte(auditLogs.timestamp, oneHourAgo)));
    ...
    async detectRateLimitViolation(ipAddress, endpoint) {
    ...
    async detectAnomalousBehavior(userId, currentActivity) {
        try {
    ...
    async getUserBehaviorProfile(userId) {
        try {
            ...
            const activities = await db
                .select()
                .from(auditLogs)
                .where(and(eq(auditLogs.userId, userId), gte(auditLogs.timestamp, thirtyDaysAgo)))
                .orderBy(desc(auditLogs.timestamp));
            if (activities.length === 0)
                return null;
            // Analyze patterns
            const accessHours = new Map();
    ...
    getThreatStatistics() {
        ...
        const last24Hours = threats.filter(t => Date.now() - t.timestamp.getTime() < 24 * 60 * 60 * 1000);
        return {
