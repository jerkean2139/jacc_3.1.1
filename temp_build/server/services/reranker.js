...
export class Reranker {
     ...
     */
    async rerank(results, query, context) {
     ...
     */
    calculateRelevanceSignals(doc, query, context) {
     ...
     */
    calculateTitleMatch(title, query) {
        if (!title)
            return 0;
        // Exact match
        if (title === query)
     ...
     */
    calculateContentMatch(content, query) {
        const queryWords = query.split(' ');
        const contentWords = content.split(' ');
        // Calculate term frequency
        let totalMatches = 0;
        for (const queryWord of queryWords) {
            const matches = contentWords.filter(word => word.toLowerCase().includes(queryWord)).length;
     ...
     */
    calculateContextMatch(doc, context) {
        if (context.length === 0)
            return 0.5;
     ...
     */
    calculateFinalScore(originalScore, signals) {
        if (!signals)
            return originalScore;
        return (this.weights.originalScore * originalScore +
