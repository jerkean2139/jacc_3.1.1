...
export class PromptChainService {
    async executeChain(query, userId, conversationHistory = []) {
        const steps = [];
        ...
        const synthesisStep = await this.synthesizeFinalResponse(query, searchStep.searchResults || [], conversationHistory, classification);
    ...
    async classifyQueryIntent(query, conversationHistory) {
        const contextHistory = conversationHistory.slice(-3).map(msg => `${msg.role}: ${msg.content}`).join('\n');
        const prompt = `
    ...
    async determineFolderRouting(classification, userId) {
        const prompt = `
    ...
    async executeSmartDocumentSearch(query, namespaces) {
        try {
            ...
            const vectorResults = searchResults.map(result => ({
                id: result.id,
    ...
    async synthesizeFinalResponse(query, searchResults, conversationHistory, classification) {
        const relevantContent = searchResults.slice(0, 5).map(result => `Document: ${result.metadata?.documentName || 'Unknown'}\nContent: ${result.content.substring(0, 500)}...`).join('\n\n');
        const contextHistory = conversationHistory.slice(-2).map(msg => `${msg.role}: ${msg.content}`).join('\n');
        const prompt = `
    ...
    buildReasoningChain(steps) {
        return steps.map(step => `Step ${step.step}: ${step.reasoning}`).join(' → ');
    }
