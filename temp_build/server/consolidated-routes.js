...
import { setupAuth, isAuthenticated, hashPassword, comparePasswords } from "./middleware/auth";
import { insertUserSchema } from "@shared/schema";
import { generateChatResponse, generateTitle } from "./services/openai";
...
import { db } from "./db";
import { eq, desc, sql, and, or, ilike, isNotNull, inArray, count } from "drizzle-orm";
import { setupOAuthHelper } from "./oauth-helper";
import { registerChatReviewRoutes } from './chat-review-routes';
import { registerSettingsRoutes } from './settings-routes';
import { validateInput, userInputSchema } from './utils/input-validation';
...
import { promptInjectionMiddleware } from './utils/prompt-injection-filter';
...
import { documents, faqKnowledgeBase, faqCategories, vendorUrls, folders, users, userStats, chats, messages, userPrompts, 
// streakTracking,
trainingInteractions, userAchievements, achievements, scheduledUrls, adminSettings } from '@shared/schema';
...
function getUltraFastResponse(message) {
...
export async function registerConsolidatedRoutes(app) {
        ...
        try {
            ...
            for (let i = 23; i >= 0; i--) {
                ...
                const [hourlyCount] = await db.select({
                    count: sql `COUNT(*)::int`
                }).from(messages).where(sql `${messages.createdAt} >= ${hourStart} AND ${messages.createdAt} < ${hourEnd}`);
    ...
    app.get('/api/chats', isAuthenticated, async (req, res) => {
        try {
            ...
            const userChats = await db.select({
                ...
                messageCount: sql `(SELECT COUNT(*) FROM ${messages} WHERE ${messages.chatId} = ${chats.id})::int`
            })
                .from(chats)
    ...
    app.post('/api/chats/:chatId/messages', isAuthenticated, async (req, res) => {
        ...
        - Keep responses concise but visually appealing with proper HTML structure.`;
                aiResponse = await fastAIService.generateFastResponse([{ role: 'user', content }], conversationPrompt);
            }
            else {
        ...
        - Keep responses professional and visually appealing with proper HTML structure.`;
                aiResponse = await fastAIService.generateFastResponse([{ role: 'user', content }], fastPrompt);
            }
...
<p>What type of business is this merchant in? I can provide more specific timeline estimates.</p>`;
        ...
        - Use <strong> for emphasis
        - Keep responses professional and visually appealing with proper HTML structure.`;
                const fastResponse = await fastAIService.generateFastResponse([{ role: 'user', content: message }], fastPrompt);
...
Return only the title, no quotes or extra text.`;
    ...
    app.post('/api/chat/send-legacy', isAuthenticated, async (req, res) => {
        try {
            ...
            const sessionToken = req.headers['x-simple-auth'] || req.cookies?.sessionToken || req.cookies?.sessionId;
        ...
        **EXAMPLES OF GOOD RESPONSES:**
        ...
        Remember: Be genuinely curious and helpful, not robotic or overly formal.`;
                response = await fastAIService.generateFastResponse([{ role: 'user', content: message }], fastPrompt);
            }
...
- Type: ${mimeType}`;
    ...
    app.get('/api/admin/system/health', async (req, res) => {
        try {
            ...
            const [dbHealth, pineconeHealth, ragHealth, cacheStats] = await Promise.allSettled([
                // Database health
    ...
    app.get('/api/admin/training-analytics', requireAdmin, async (req, res) => {
        try {
            const [counts] = await db.select({
                totalInteractions: sql `COUNT(*)::int`,
                totalMessages: sql `COUNT(DISTINCT query)::int`,
                flaggedForReview: sql `SUM(CASE WHEN was_correct = false THEN 1 ELSE 0 END)::int`
    ...
    function enhanceResponseWithHTML(content) {
    ...
    app.get('/api/admin/live-chats', requireAdmin, async (req, res) => {
        try {
            const recentChats = await db.select({
                ...
                userId: chats.userId,
                title: chats.title,
                messageCount: sql `COUNT(${messages.id})::int`,
    ...
    app.get('/api/admin/system-metrics', requireAdmin, async (req, res) => {
        try {
            ...
            const [chatCount] = await db.select({ count: sql `COUNT(*)::int` }).from(chats);
            const [messageCount] = await db.select({ count: sql `COUNT(*)::int` }).from(messages);
            const [documentCount] = await db.select({ count: sql `COUNT(*)::int` }).from(documents);
            res.json({
    ...
    app.get('/api/admin/documents/search', requireAdmin, async (req, res) => {
        try {
            ...
            const searchResults = await db.select()
                .from(documents)
                .where(or(ilike(documents.name, `%${query}%`), ilike(documents.content, `%${query}%`)))
                .limit(50);
    ...
    */
    ...
    app.get('/api/leaderboard', async (req, res) => {
        try {
            ...
            const leaderboardQuery = await db
                ...
                chatCount: sql `coalesce((select count(*) from ${chats} c where c.user_id = ${users.id}), 0)`,
                messageCount: sql `coalesce((select count(*) from ${messages} m join ${chats} c on m.chat_id = c.id where c.user_id = ${users.id}), 0)`,
    ...
    app.put('/api/admin/users/:id', requireAdmin, async (req, res) => {
        try {
            ...
            if (updateData.isoHubId !== undefined)
                updates.isoHubId = updateData.isoHubId;
            ...
            if (updates.username || updates.email) {
                const conflicts = await db.select()
                    .from(users)
                    .where(and(sql `${users.id} != ${userId}`, or(updates.username ? eq(users.username, updates.username) : sql `false`, updates.email ? eq(users.email, updates.email) : sql `false`)));
        ...
        try {
            ...
            if (existingUser.role === 'dev-admin' || existingUser.role === 'client-admin') {
                const adminCount = await db.select({ count: sql `COUNT(*)::int` })
                    .from(users)
                    .where(and(or(eq(users.role, 'dev-admin'), eq(users.role, 'client-admin')), eq(users.isActive, true)));
    ...
    app.get('/api/admin/audit-logs', requireAdmin, async (req, res) => {
        try {
            const { startDate, endDate, eventType, userId, ipAddress, limit = 100, offset = 0 } = req.query;
            const options = {
        ...
        try {
            ...
            const dashboard = {
                ...
                alerts: {
                    ...
                    suspiciousActivity: securityEvents.filter(e => e.eventType === AuditEventType.SUSPICIOUS_ACTIVITY).length,
                    rateLimitViolations: securityEvents.filter(e => e.eventType === AuditEventType.RATE_LIMIT_EXCEEDED).length,
    ...
    app.get('/api/debug/auth-status', async (req, res) => {
        try {
            ...
            const testUsers = await db.select({
                ...
                .where(or(eq(users.username, 'admin'), eq(users.username, 'tracer-user'), eq(users.username, 'manager'), eq(users.username, 'cburnell')));
    ...
    app.get('/api/folders', async (req, res) => {
        try {
            ...
            const foldersWithCounts = await Promise.all(foldersList.map(async (folder) => {
                try {
    ...
    app.get('/api/admin/ocr/queue', requireAdmin, async (req, res) => {
        try {
            ...
            const docs = await db.query.documents.findMany({
                where: or(eq(documents.mimeType, 'application/pdf'), ilike(documents.mimeType, 'image/%')),
                orderBy: [desc(documents.createdAt)],
                limit: 50
            });
    ...
    app.post('/api/admin/ocr/process-document/:id', requireAdmin, async (req, res) => {
        try {
            ...
            let ocrResult = { text: '', confidence: 0, method: 'none' };
...
This may indicate the OCR service is not properly configured or the file format is not supported`;
    ...
    app.post('/api/admin/ocr/batch-process', requireAdmin, async (req, res) => {
        try {
            ...
            const docs = await db.select()
                .from(documents)
                .where(and(sql `${documents.id} = ANY(${documentIds})`, or(eq(documents.mimeType, 'application/pdf'), ilike(documents.mimeType, 'image/%'))));
    ...
    app.get('/api/admin/ocr/quality-metrics', requireAdmin, async (req, res) => {
        try {
            ...
            }).from(documents).where(or(eq(documents.mimeType, 'application/pdf'), ilike(documents.mimeType, 'image/%')));
            const [processedDocsResult] = await db.select({
                count: count(documents.id)
            }).from(documents).where(and(or(eq(documents.mimeType, 'application/pdf'), ilike(documents.mimeType, 'image/%')), isNotNull(documents.content)));
        ...
        try {
            ...
            res.json({
                ...
                services: {
                    ...
                    ai: unifiedAIService ? 'connected' : 'disconnected',
                    batch: batchProcessor.getStats().totalJobs > 0 ? 'active' : 'ready',
                    vectorService: { service: 'unknown', status: 'error', error: error.message }
                }
            });
        }
    });
    ...
    app.post('/api/admin/compliance/soc2/generate', async (req, res) => {
        try {
            ...
            const report = await complianceReportingService.generateSOC2Report(new Date(startDate), new Date(endDate), generatedBy);
            res.json(report);
        }
        catch (error) {
    ...
    app.get('/api/usage/stats', isAuthenticated, async (req, res) => {
        try {
            ...
            const { apiCostTracker } = require('./services/api-cost-tracker');
            const stats = await apiCostTracker.getUserUsageStats(userId, year ? parseInt(year) : undefined, month ? parseInt(month) : undefined);
            res.json(stats);
        }
        catch (error) {
