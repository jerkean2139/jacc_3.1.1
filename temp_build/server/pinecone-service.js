    ...
    async search(query, limit = 10, threshold = 0.7) {
        ...
        try {
            ...
            return queryResponse.matches
                ...
                metadata: {
                    documentName: match.metadata?.documentName || `Document ${match.id}`,
                    webViewLink: match.metadata?.webViewLink || `/documents/${match.id}`,
                    chunkIndex: match.metadata?.chunkIndex || 0,
                    mimeType: match.metadata?.mimeType || 'application/pdf'
                }
            })) || [];
        }
        catch (error) {
