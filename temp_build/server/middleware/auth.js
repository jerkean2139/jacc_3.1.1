...
const sessionRotationTracker = new Map();
...
async function checkSessionRotation(req, res) {
...
async function updateSessionActivity(req) {
    const sessionId = req.cookies?.sessionToken || req.cookies?.sessionId || req.session?.id;
    if (!sessionId)
        return;
    const tracker = sessionRotationTracker.get(sessionId);
...
export async function recordFailedLogin(identifier, req) {
    const now = Date.now();
    const attempts = failedLoginAttempts.get(identifier) || {
        count: 0,
        firstAttempt: now
