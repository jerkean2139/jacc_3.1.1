...
export class AdvancedSearchService {
  ...
  async searchDocuments(query: string, limit: number = 20): Promise<EnhancedSearchResult[]> {
    try {
      // Expand query terms
      ...
      const enhancedResults: EnhancedSearchResult[] = keywordResults.map((chunk, index) => {
        const keywordMatches = expandedTerms.filter(term =>
  ...
  private extractContextualInfo(content: string, terms: string[]): string {
    const contentLower = content.toLowerCase();
